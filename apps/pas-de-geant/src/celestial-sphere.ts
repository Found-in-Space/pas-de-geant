...
class CelestialPlanetField {
  ...
  private readonly apparentMagnitudes = new Float64Array(
  ...
  updateEphemeris(planets: readonly CelestialPlanetEphemeris[]): void {
    for (const planet of planets) {
      ...
      this.apparentMagnitudes[index] = planet.apparentMagnitude;
      this.syncVisibility(index);
  ...
  private syncVisibility(index: number): void {
    this.sprites[index]!.visible =
      this.enabled[index]! &&
      this.apparentMagnitudes[index]! <= CELESTIAL_LIMITING_MAGNITUDE;
  }
