...
import type {
  ...
  TileProviderResult,
  TileRequestHandle,
} from "./construct-tile-provider.js";

export type ConstructImageMode = "imagery" | "terrain";
export type ConstructImageCacheStatus =
  | "memory"
  | "shared"
  | "provider"
  | "persistent-hit"
...
export interface ConstructImageTileResource {
  readonly kind: "image";
  readonly mode: ConstructImageMode;
  ...
  readonly cacheStatus: ConstructImageCacheStatus;
  readonly loadDurationMs: number;
  readonly byteLength: number;
}

export interface ConstructImageProviderMetrics {
...
interface LoadedSource {
  readonly image: HTMLImageElement;
  readonly byteLength: number;
  readonly cacheStatus: Exclude<ConstructImageCacheStatus, "memory" | "shared">;
  readonly loadDurationMs: number;
}

export interface ConstructSourceLoad {
  readonly image: HTMLImageElement;
  readonly byteLength: number;
  readonly cacheStatus: LoadedSource["cacheStatus"];
}

export interface ConstructImageTileProviderOptions {
  readonly mode: ConstructImageMode;
  readonly tilePixels: number;
...
interface Consumer {
  ...
  readonly observer: (
    result: TileProviderResult<ConstructImageTileResource>,
  ) => void;
  active: boolean;
}
...
/**
 * Adapts real image loaders to the Construct's cancellable tile protocol. It
 * coalesces identical overzoomed source requests and retains decoded sources so
 * mode switching and revisits expose memory-cache behaviour directly.
 */
export class ConstructImageTileProvider
  implements TileProvider<ConstructImageTileResource>
{
  readonly mode: ConstructImageMode;
  readonly tilePixels: number;
  readonly attribution: string;
  ...
  private readonly listeners = new Set<
    (metrics: ConstructImageProviderMetrics) => void
  >();
  ...
  constructor(private readonly options: ConstructImageTileProviderOptions) {
    this.mode = options.mode;
    this.tilePixels = options.tilePixels;
    this.attribution = options.attribution ?? "";
  }

  get metrics(): ConstructImageProviderMetrics {
    return Object.freeze({
  ...
  subscribe(
    listener: (metrics: ConstructImageProviderMetrics) => void,
  ): () => void {
  ...
  private resourceFor(
    consumer: Consumer,
    source: LoadedSource,
    cacheStatus: ConstructImageCacheStatus,
  ): ConstructImageTileResource {
    return Object.freeze({
      kind: "image",
      mode: this.mode,
      tile: consumer.tile,
...
export function constructImageryProvider(
  provider: ImageryProvider,
): ConstructImageTileProvider {
  return new ConstructImageTileProvider({
    mode: "imagery",
    ...
    resolveSource: (tile) => {
      ...
      const source = ancestorAtZoom(
        tile as ImageryAddress,
        Math.min(tile.z, provider.maxZoom),
      );
...
export function constructTerrainProvider(): ConstructImageTileProvider {
  return new ConstructImageTileProvider({
    mode: "terrain",
    tilePixels: LOCAL_TILE_SIZE,
    attribution: "Mapterhorn · Terrarium elevation tiles",
    concurrency: 4,
    resolveSource: (tile) => sourceMapping(tile, terrainSourceAddress(tile)),
    loadSource: async (source, signal) => {
      ...
      try {
        return {
          image: await decodeBlobImage(blob, LOCAL_TILE_SIZE, signal),
