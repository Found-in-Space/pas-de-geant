...
} from "../apps/pas-de-geant/src/construct-image-tile-provider.js";
import type { TileProviderResult } from "../apps/pas-de-geant/src/construct-tile-provider.js";

describe("The Construct real image tile provider", () => {
  it("starts the highest display zoom first", async () => {
    const startedZooms: number[] = [];
    const provider = new ConstructImageTileProvider({
      mode: "imagery",
      tilePixels: 512,
      concurrency: 1,
      resolveSource: (tile) => ({
  ...
  it("coalesces overzoomed children and then serves revisits from decoded memory", async () => {
    ...
    const resources: ConstructImageTileResource[] = [];
    const observe = (
